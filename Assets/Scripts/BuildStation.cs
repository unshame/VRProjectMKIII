...
public class BuildStation : MonoBehaviour {
    ...
    void Start() {
        ...
        //blockSize = baseBlock.GetComponent<Renderer>().bounds.size.x;
        ...
        for (int x = 0; x < size; x++) {
            ...
            for (int y = 0; y < size; y++) {
                ...
                for (int z = 0; z < size; z++) {
                    var block = new Block(new Vector3(position.x + x * blockSize, position.y + y * blockSize, position.z + z * blockSize), Quaternion.identity, transform.parent);
                    axis.Add(block);
                }
                dimension.Add(axis);
            }
    ...
    void AddBlock(Coord blockCoord, GameObject otherBlock, Vector3 offset, List<Block> affectedBlocks) {
        var block = blocks[blockCoord.x][blockCoord.y][blockCoord.z];
